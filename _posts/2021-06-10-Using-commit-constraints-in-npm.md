...
You could require the main branch instead, and get those changes:

```shell
npm install git:github.com/vendor/package-name#main
```

This is not recommended however. Any update after the change from the main branch you need gets installed, even the changes you don't need. And whenever a broken commit gets merged you will install that broken code on the next update - even when that broken code is not tagged for release. 
